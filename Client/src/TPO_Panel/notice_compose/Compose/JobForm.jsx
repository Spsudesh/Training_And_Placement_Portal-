      ...
      <div className="grid gap-5 md:grid-cols-2">
        ...
        <Field label="CTC">
          <input
            type="text"
            value={formData.ctc}
            onChange={(event) => onFieldChange("ctc", event.target.value)}
            placeholder="8.5 LPA"
            className={inputClassName()}
          />
        </Field>
      ...
      <Field label="Hiring Process">
        <div className="space-y-3">
          {formData.hiringProcess.map((stage, index) => (
            <div key={`${stage}-${index}`} className="flex gap-3">
              ...
              <button
                ...
                className="rounded-2xl border border-rose-200 px-4 py-3 text-sm font-medium text-rose-600 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-50"
          ...
          <button
            type="button"
            onClick={onAddStage}
            className="rounded-2xl border border-dashed border-cyan-300 px-4 py-3 text-sm font-semibold text-cyan-700 transition hover:bg-cyan-50"
          >
            Add Stage
          </button>
        </div>
