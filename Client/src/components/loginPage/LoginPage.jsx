...
import { loginUser } from "../../shared/authApi";
...
function getLoginPayloadRole(role) {
  return role;
}
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await loginUser({
        email,
        password,
        role: getLoginPayloadRole(selectedRole),
      });
      ...
      onLogin?.(panel, studentId, response?.user?.email || email);
        ...
        <div className="w-full rounded-[28px] border border-[#dce6f3] bg-white px-6 py-5 shadow-[0_24px_60px_rgba(15,23,42,0.08)] sm:px-8">
          ...
          <form className="mt-5 space-y-4" onSubmit={handleSubmit}>
            <div>
              ...
              <div className="grid grid-cols-3 rounded-2xl bg-[#eef3fb] p-1.5">
            ...
            <div>
              <label htmlFor="login-user-id" className="mb-2 block text-sm font-medium text-slate-700">
                Email
              </label>
              <input
                id="login-email"
          ...
          <p className="mt-4 text-center text-sm text-slate-500">
            Need a new account?{" "}
            <Link to="/signup" className="font-semibold text-[#0b7ca0] hover:text-[#075f7a]">
              Sign up
            </Link>
          </p>
        </div>
